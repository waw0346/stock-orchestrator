...
17. **대화 중 주식 일정 실시간 추출 프로토콜**:
    - 사용자 대화나 에이전트 분석 내용 중 구체적인 날짜(예: "6월 15일", "2026-06-12", "내일" 등)와 주식 관련 주요 이벤트(락업 해제, 실적 발표, CPI/FOMC 일정, 배당금 등)가 감지되면 즉시 이를 포착합니다.
    - 해당 날짜의 일간 캘린더 파일인 `11_calendar/daily/YYYY-MM-DD Stock Calendar.md`가 존재하는지 확인하고, 없다면 `Stock Calendar Day Template.md`를 사용하여 파일을 새로 생성합니다.
    - 파일 내부의 `## 오늘 일정` 하위 테이블(Table) 양식에 맞추어 아래 형식의 행을 자동 기입(Append)하여 기록을 보존합니다:
      `| 시간 | 분류 | 종목/시장 | 일정 | 중요도 | 상태 | 관련 노트 |`
      예: `| 장시작 | 공시/이벤트 | 티엠씨 (217590) | 보호예수(17.5%) 물량 해제 | 높음 | active | [[07_stock_analysis/2026-06-09_217590]] |`
18. **세션 대화 및 작업 타임라인 연동 프로토콜**: 사용자 대화 세션 종료 시, 당일의 대화 요약, 사용자 질문, 주요 결정사항, 실행된 작업 목록을 일별/시간별 타임라인(`01_daily_logs/YYYY-MM-DD Daily Log.md`)의 '시간별 작업 및 대화 타임라인 (Timeline)' 섹션에 타임스탬프와 함께 연동하여 기록을 업데이트합니다.
