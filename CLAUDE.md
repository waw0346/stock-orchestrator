...
큰 로컬 캐시를 읽어야 할 때는 `python scripts/summarize_context.py --ticker [종목코드] --purpose risk|flow|market`로 필요한 종목·목적만 projection한 뒤 전달한다.
...
다른 AI 런타임에서 실행할 때는 `AGENTS.md`와 `docs/ai_runtime_adapter.md`의 도구 매핑을 따른다. 핵심 계약은 역할 분리, 작은 JSON 요약, 검증 가능한 출처이며 특정 도구명 자체가 아니다.
